...
def test_key_merge(tmp_path):
    ...
    fns = [
        str(p)
        for p in pathlib.Path(fixture_path + "raw").glob("**/*")
...
def test_refit(tmp_path):
    ...
    p = Pipe(fns).map(suite.refit, kwargs={"output_path": tmp_path}, eager=True)
...
def test_image_grind(tmp_path):
    ...
    p = Pipe(fns).map(
        ...
        kwargs={"output_path": tmp1, "tmp_path": tmp0},
        eager=True,
    )
...
def test_paste_mark_recorder(tmp_path):
    ...
    fns = [
        str(p)
        for p in pathlib.Path(fixture_path + "topaste_mark_recorder").glob("**/*")
    ]

    p = Pipe(fns).map(
        suite.paste_markrecorder, kwargs={"output_path": tmp_path}, eager=True
    )
...
def test_read_id_marks(tmp_path):
    ...
    p = Pipe(fns).map(suite.read_id_marks, eager=True)
...
def test_read_bubble_array(tmp_path):
    ...
    fns = [
        str(p) for p in pathlib.Path(fixture_path + "toread_mark_recorder").glob("**/*")
    ]
